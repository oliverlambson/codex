...
#[tokio::test]
async fn height_shrink_during_agent_stream_requests_post_consolidation_reflow() {
    let (mut app, _rx, _op_rx) = make_test_app_with_channels().await;
    let frame_requester = crate::tui::FrameRequester::test_dummy();
    app.transcript_cells = vec![Arc::new(AgentMessageCell::new(
        vec![Line::from("transient streamed response")],
        /*is_first_line*/ true,
    ))];
    ...
    assert!(
        ...
        "height-only resize must be repaired from the consolidated source cell"
    );
}
